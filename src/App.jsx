...
const App = () => {
  ...
  const AuthScreen = ({ initialMode }) => {
    ...
    return (
      <div className="flex flex-col min-h-screen bg-beige p-6 relative animate-fade-in">
        ...
        <div className="flex-1 flex flex-col justify-center max-w-sm w-full mx-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-4 mt-6 bg-navy text-white rounded-2xl font-black text-lg shadow-lg shadow-navy/20 active:scale-95 transition-all flex items-center justify-center gap-2 disabled:opacity-50"
            >
  ...
  const ExamResultsScreen = () => {
    ...
    return (
      <div className="min-h-screen bg-beige flex flex-col items-center p-6 text-center pb-24 relative overflow-y-auto animate-fade-in">
        <div className="w-32 h-32 relative flex items-center justify-center mb-6 mt-8 animate-scale-up">
        ...
        <h2 className="text-3xl font-black text-navy mb-2">¡Examen Finalizado!</h2>
        <p className="text-teal mb-8 font-semibold">Acertaste {score} de {questions.length} preguntas de {selectedSubject?.name}.</p>
        ...
        {examMode === 'diagnostic' && failedCategories.length > 0 && (
          <div className="max-w-md w-full mb-8">
            <div className="bg-white p-5 rounded-2xl border border-sky-blue/20 shadow-sm text-left">
              <span className="text-[10px] font-black text-teal uppercase tracking-widest block mb-2">Temas a reforzar:</span>
              <div className="flex flex-wrap gap-2 mb-4">
                {failedCategories.map(cat => (
                  <span key={cat} className="px-3 py-1 bg-sky-blue/20 text-navy rounded-lg text-xs font-bold capitalize border border-sky-blue/20">{cat}</span>
                ))}
              </div>

              <div className="mt-6 pt-4 border-t border-sky-blue/10">
                <p className="text-xs text-teal/80 font-semibold leading-relaxed">Tus 2 exámenes de práctica personalizados ya han sido generados y están listos en la pestaña de materias. ¡Haz clic en Continuar Estudiando para reforzarlos!</p>
              </div>
            </div>
          </div>
        )}

        <button onClick={() => setScreen('subject')} className="w-full max-w-md py-4 bg-navy text-white rounded-2xl font-black shadow-lg shadow-navy/20 active:scale-95 transition-all fixed bottom-6 left-1/2 -translate-x-1/2 z-10 hover:bg-navy/95">
