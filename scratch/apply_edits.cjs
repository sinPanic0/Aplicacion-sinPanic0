          ...
          <div className="flex justify-between relative z-10">
            {[1, 2, 3, 4].map(level => (
              <button
                ...
                className={\`w-5 h-5 rounded-full border-2 transition-all duration-300 disabled:opacity-50 \&{userProfile.intensity >= level ? 'bg-teal border-white shadow-md' : 'bg-white border-sky-blue/30'}\`.replace('\&', '$')}
              />
            ))}
          </div>
        </div>
